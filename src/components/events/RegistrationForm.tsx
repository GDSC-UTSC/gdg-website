...
export default function RegistrationForm({ event }: RegistrationFormProps) {
 ...
 const [isRegistrantSpeaker, setIsRegistrantSpeaker] = useState("");
       ...
       <form onSubmit={handleFormSubmit} className="space-y-8">
         ...
         <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
           <div className="bg-muted/30 p-6 rounded-lg border border-border/50">
            ...
            <div className="space-y-2">
                 <Label htmlFor="isRegistrantSpeaker">
                   Are you a speaker? <span className="text-red-500">*</span>
                 </Label>
                 <div className="flex items-center space-x-4">
                 <label className="flex items-center">
                   <input
                     id="isRegistrantSpeakerYes"
                     type="radio"
                     name="isRegistrantSpeaker"
                     value="yes"
                     checked={isRegistrantSpeaker === "yes"}
                     onChange={(e) => setIsRegistrantSpeaker(e.target.value)}
                     className="mr-2"
                   />
                   Yes
                 </label>
               ...
               <label className="flex items-center">
                 <input
                   id="isRegistrantSpeakerNo"
                   type="radio"
                   name="isRegistrantSpeaker"
                   value="no"
                   checked={isRegistrantSpeaker === "no"}
                   onChange={(e) => setIsRegistrantSpeaker(e.target.value)}
                   className="mr-2"
                 />
                 No
           ...
           {isRegistrantSpeaker === "yes" && (
             <div className='space-y-2 mt-4'>
               <Label htmlFor="encryptedHtml">
                 Please enter your speaker code <span className="text-red-500">*</span>
               </Label>
               <Input
                 id="encryptedHtml"
                 type="password"
                 value={encryptedHtml}
                 onChange={(e) => {
                   setEncryptedHtml(e.target.value);
                   if (errors.encryptedHtml) {
                     setErrors((prev) => ({ ...prev, encryptedHtml: "" }));
                   }
                 }}
                 placeholder="Enter your speaker code"
                 className={errors.encryptedHtml ? "border-red-500" : ""}
               />
             </div>
           )}
         </motion.div>
